  ...
  >
    <el-row>
          ...
          >
            <template #default>
              <el-tree-select
                ...
                :key="item.nodeKey"
                v-model="props.formField[item.prop]"
                :data="item.data"
                :lazy="item.lazy"
                :load="item.load"
              ...
              <el-input
                ...
                :type="passwordHide ? item.inpType || 'text' : 'text'"
                show-word-limit
                @change="
                  () => {
              ...
              <upload-img
                ...
                v-model:image-url="props.formField[item.prop]"
                :height="item.height || '8rem'"
                :width="item.width || '8rem'"
              />
              <el-switch
                v-if="item.type === 'switch'"
                v-model="props.formField[item.onOff]"
              ...
              <div
                ...
                @click="
                  () => {
                    if (item.onClick) item.onClick(props.formField)
                  }
                "
              >
      ...
      <template v-for="btn in testCaseConfig.buttons" :key="btn.icon">
        <el-col :span="24 / testCaseConfig.buttons.length">
          <el-button
            :type="btn.type"
            :size="btn.size"
            @click="
              () => {
                if (item.onClick) item.onClick(item.generateTestCase)
              }
            "
          >
            {{ btn.name }}
